;(function ($, window, document, undefined) {
    ...
    var DOCUMENT = $(document),
        ...
        GB = $.modalBox = function () {
            GB.open.apply(this, arguments);
        },

        /**
        * Функция определяет является ли объект Jquery объектом
        */
    ...
    $.extend(GB, {
        ...
        theme: {
            defaults: [
                ...
                '<div class="modal-close js-modal_close"></div>',
                '</div>',
                '<div class="modal-content"></div>'
            ].join(''),

            blank: [
                '<div class="modal-content modal-bg-transparent"></div>'
            ].join(''),

            image: [
                '<div class="modal-gallery-content"></div>',
                '<div class="modal-gallery-next"></div>',
                '<div class="modal-gallery-prev"></div>',
                '<div class="modal_close-top js-modal_close"></div>'
            ].join('')
        },
        options: {},
        element: {},
        ...
        open: function (el, options) {
            ...
            $.extend(true, GB.options, GB.defaults, options || {});

            if (isQuery(el)) {
                obj = {
                    el: el,
                    href: el.data('gb-href') || el.attr('href') || '#' + el.attr('id')
                };

                $.extend(GB.options, el.html5data('gb'), obj);

                if (!el.data('gb-theme') && (GB.options.type === 'inline' || GB.options.type === 'ajax')) {
                    GB.options.theme = 'defaults';
                }
            } else {
                $.extend(GB.options, el);

                if (!el.theme && (GB.options.type === 'inline' || GB.options.type === 'ajax')) {
                    GB.options.theme = 'defaults';
                }
            }
            ...
            GB.container.off('.gb-close').on('click.gb-close', '.js-modal_close', function (e) {
                e.preventDefault();
        ...
        close: function () {
            if (GB.options.type === 'inline') {
                GB.dataHtml.trigger('modal-insert');
            }
        ...
        inline: function () {
            ...
            if (GB.dataHtml.length) {
                content = GB.container.find('.modal-content');

                GB.tmp = $('<div/>', {
                    'class': 'modal-tmp-inline'
                });
                GB.tmp.insertBefore(GB.dataHtml);

                GB.dataHtml.off('modal-insert').on('modal-insert', function () {
                    GB.dataHtml.insertBefore(GB.tmp).removeClass('modal-show-all');
                ...
                content
                    .empty()
                    .append(GB.dataHtml.addClass('modal-show-all'));
         ...
         */
        image: function () {
            var isGallery = GB.options.el.data('gb-gall') || false;

            var items = (isGallery) ? DOCUMENT.find('a[data-gb-gall=' + isGallery + ']') : GB.options.el,
                index = items.index(GB.options.el);

            var prev, next, gallery;

            GB.container.empty().append(GB.getTheme());

            gallery = GB.container.find('.modal-gallery-content');

            prev = GB.container.find('.modal-gallery-prev');
            next = GB.container.find('.modal-gallery-next');

            loadImage(GB.options.el, index);
            ...
            function loadImage(el, index) {
                ...
                image.onload = function () {
                    ...
                    container = GB.container.find('.modal-gallery-content');
                    container.css({
                        'width': size.width,
                        'height': size.height
                    }).empty().append(image);
